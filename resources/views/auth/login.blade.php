...
<body class="bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 min-h-screen transition-colors duration-300">
    ...
    <div class="min-h-screen flex flex-col md:flex-row">
        ...
        {{-- ── Kolom Kanan: Form Login (60% Desktop) ── --}}
        <div class="w-full md:w-[60%] bg-white dark:bg-gray-800 p-8 md:p-16 flex flex-col justify-center relative">
            <div class="max-w-md w-full mx-auto space-y-8">
                
                {{-- Header Form --}}
                <div class="space-y-2">
                    <h1 class="text-3xl font-black text-gray-900 dark:text-white">Selamat Datang Kembali 👋</h1>
                    <p class="text-sm text-gray-500 dark:text-gray-400">Masuk ke akun LaporHijau untuk mulai berkontribusi</p>
                </div>

                {{-- Alert Status --}}
                @if (session('status'))
                    <div class="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 text-green-800 dark:text-green-300 text-sm rounded-xl p-4">
                        {{ session('status') }}
                    </div>
                @endif
                ...
                <form method="POST" action="{{ route('login') }}" class="space-y-5">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <div class="relative">
                            ...
                            <input id="email" type="email" name="email" value="{{ old('email') }}" required autofocus
                                   ...
                                   class="w-full pl-10 pr-4 py-3 border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent transition">
                        </div>
                        @error('email')
                    ...
                    <div class="space-y-1.5">
                        <div class="flex items-center justify-between">
                            ...
                            @if (Route::has('password.request'))
                                <a href="{{ route('password.request') }}" class="text-xs text-green-600 hover:text-green-700 font-bold">Lupa Sandi?</a>
                            @endif
                        </div>
                        <div class="relative">
                    ...
                    <div class="flex items-center">
                        <input id="remember_me" type="checkbox" name="remember"
                               class="rounded border-gray-300 dark:border-gray-700 text-green-600 shadow-sm focus:ring-green-500 w-4 h-4 bg-gray-50 dark:bg-gray-900">
                        <label for="remember_me" class="ms-2 text-xs font-semibold text-gray-500 dark:text-gray-400">Tetap masuk di perangkat ini</label>
                    </div>

                    {{-- Submit Button --}}
                    <button type="submit"
                            class="w-full py-3 px-4 bg-green-600 hover:bg-green-700 text-white font-bold rounded-xl text-sm transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-2">
                        Masuk Sekarang →
                    </button>
                </form>
                ...
                <div class="text-center pt-4 border-t border-gray-100 dark:border-gray-700">
                    <p class="text-xs text-gray-500 dark:text-gray-400">
                        Belum punya akun LaporHijau? 
                        <a href="{{ route('register') }}" class="text-green-600 hover:text-green-700 font-bold ml-1">Daftar sekarang gratis</a>
                    </p>
                </div>
