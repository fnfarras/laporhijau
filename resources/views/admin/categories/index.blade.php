                ...
                <form method="POST" action="{{ route('admin.categories.store') }}" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name') }}" required
                               ...
                               class="w-full px-3 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-red-300 @error('name') border-red-400 @enderror">
                    ...
                    <div>
                        ...
                        <input type="text" name="icon" value="{{ old('icon') }}" required maxlength="10"
                               placeholder="🗑️"
                               class="w-full px-3 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-red-300 @error('icon') border-red-400 @enderror">
                        @error('icon') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>
                    <button type="submit" class="w-full py-2.5 bg-red-600 hover:bg-red-700 text-white text-sm font-bold rounded-xl transition-colors">
                        + Tambah Kategori
                    </button>
                </form>
            </div>
                ...
                <div class="divide-y divide-gray-50">
                        ...
                        <div class="flex items-center gap-4 px-5 py-3.5">
                            ...
                            <form method="POST" action="{{ route('admin.categories.update', $category) }}"
                                ...
                                <input type="text" name="icon" value="{{ $category->icon }}" maxlength="10"
                                       class="w-12 px-2 py-1.5 border border-gray-200 rounded-lg text-sm text-center focus:outline-none focus:ring-1 focus:ring-red-400">
                                <input type="text" name="name" value="{{ $category->name }}"
                                       class="w-40 px-3 py-1.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-red-400">
                    ...
                    @empty
                        <div class="p-16 text-center">
                            ...
                            <p class="text-sm font-bold text-gray-600">Belum ada kategori</p>
                        </div>
                    @endforelse
                </div>
            </div>
